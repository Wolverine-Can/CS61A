...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return (is_weight(end(left(m))) or balanced(end(left(m)))) and (is_weight(end(right(m))) or balanced(end(right(m)))) and (total_weight(end(left(m)))*length(left(m)) == total_weight(end(right(m)))*length(right(m)))
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attemps=[]
    def withdraw(amount,verify_password):
        nonlocal balance,attemps
        if len(attemps) == 3:
            return 'Your account is locked. Attempts: '+str(attemps)
        elif verify_password != password:
            attemps = attemps + [str(verify_password)]
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(error) == str:
        return error
    def withdraw2(amount, verify_password):
        if verify_password == old_password or verify_password == new_password:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, verify_password)
    return withdraw2
