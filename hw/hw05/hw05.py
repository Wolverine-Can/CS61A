...
class VendingMachine:
    ...
    def restock(self, number):
        self.stock += number
        return 'Current {0} stock: {1}'.format(str(self.item),str(self.stock))
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            if self.balance == 0:
                return 'Here is your {0}.'.format(str(self.item))
            else:
                return_money, self.balance = self.balance, 0
                return 'Here is your {0} and ${1} change.'.format(str(self.item),str(return_money))
